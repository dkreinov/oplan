...
def accepted_worktree_state(repo: Path, relative: str) -> str:
    ...
    for descendant in descendants:
        ...
        # D-005/D-027: tool caches are regenerated by running validation, not
        # executor writes; excluding them from the accepted-state digest keeps
        # this side blind to the same bytes the undeclared-write comparison
        # already ignores, so a fresh __pycache__/.pytest_cache/.pyc/.pyo does
        # not read as drift.
        if tool_cache(descendant_relative):
            continue
